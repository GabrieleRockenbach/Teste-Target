...
function calcLowestMonthlyRevenue (data) {
    const lowestMonthlyRevenue = Math.min(...data.map((element) => element.valor).filter((element)=> element != 0));
    const indexOfDay = (data.find((element) => element.valor == lowestMonthlyRevenue)).dia;
    return [lowestMonthlyRevenue , indexOfDay];
}
function calcHighestMonthlyRevenue (data) {
    const highestMonthlyRevenue = Math.max(...data.map((element) => element.valor));
    const indexOfDay = (data.find((element) => element.valor == highestMonthlyRevenue)).dia;
    return [highestMonthlyRevenue , indexOfDay];
}
function calcMediaOfMonth(data){
    const validDaysOfMonth = data.filter((element)=> element.valor != 0).length;
    const total = data.map((element)=> element.valor).reduce((total , value) => total + value , 0);
...
function processRevenueData (data) {
    ...
    highestRevenueButton.addEventListener('click', () => {
        ...
        highestRevenueButton.innerText = `Maior faturamento: R$ ${result[0].toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}\nDia: ${result[1]}`
    })
    
    lowestRevenueButton.addEventListener('click', () => {
        const result = calcLowestMonthlyRevenue(data);
        lowestRevenueButton.innerText = `Menor faturamento: R$ ${result[0].toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}\nDia: ${result[1]}`
    })

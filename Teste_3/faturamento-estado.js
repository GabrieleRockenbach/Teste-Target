...
function calcTotalRevenue (data) {
    const totalRevenue = data.map((element) => element.revenue).reduce((total , currentValue) => total + currentValue);
    return totalRevenue;
}
function calcPercentage(data) {
    data.forEach(element => {
        element["percentage"] = (((element.revenue) / (calcTotalRevenue(data))) * 100).toFixed(1);
    });
}
